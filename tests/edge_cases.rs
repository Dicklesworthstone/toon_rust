...
use toon_rust::{JsonValue, decode, encode, try_decode};
...
fn numeric_nan_becomes_null() {
    ...
    let value = JsonValue::Object(vec![
        (
            "nan".to_string(),
            JsonValue::Primitive(toon_rust::StringOrNumberOrBoolOrNull::from_f64(f64::NAN)),
        ),
        (
            "valid".to_string(),
            JsonValue::Primitive(toon_rust::StringOrNumberOrBoolOrNull::Number(42.0)),
        ),
    ]);
    let toon = toon_rust::encode::encode(value, None);
    let decoded = decode(&toon, None);
...
fn numeric_infinity_becomes_null() {
    let value = JsonValue::Object(vec![
        (
            "pos_inf".to_string(),
            JsonValue::Primitive(toon_rust::StringOrNumberOrBoolOrNull::from_f64(
                f64::INFINITY,
            )),
        ),
        (
